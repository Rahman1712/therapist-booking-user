       ...
       {roomId ? 
        ...
        <div className="grid grid-cols-12 gap-y-2">
          {messages.map((message) => (
            <ChatMessageItem
              key={message.id}
              isself={message.role == "THERAPIST"}
              icon={message.role == "THERAPIST" ? therapistAvatar : userAvatar }
        ...
        :
        <div className="text-center flex justify-center items-center mt-10">
          <span className="text-cyan-900 font-roboto-mono">select a user to chat</span>
        </div>
        }
      </div>
    </div>
