...
import { getAvatarTherapist, getUserAvatar } from '../../../utils/AvatarUtils';

const ChatMessagesSide = ({ sendMsg, setNewMessage, messages }) => {
  const [therapistColors, setTherapistColors] = useState({}); // Maintain user colors
  const inputRef = useRef(null);

  const { activeTherapist } = useSelector((state) => state.chat);
  const { fullname, imageUrl } = useSelector((state) => state.auth);

  const therapistAvatar = getAvatarTherapist(activeTherapist, therapistColors, setTherapistColors);
      ...
      {activeTherapist &&
        <div className="flex flex-row items-center py-4 px-6 rounded-2xl shadow">
          ...
          <div className="flex flex-col ml-3">
            <div className="font-semibold text-sm">{activeTherapist?.fullname}</div>
